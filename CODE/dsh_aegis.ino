...
#include "dsh_icons.h"   // boot logo + Flipper-style pixel icons
...
static const char*    SETTINGS_PATH  = "/AEGIS.CFG";   // non-secret prefs
...
// These start at sensible defaults but are editable in the Settings screen and
// saved to SD, so they survive a reboot. Compile-time constants can't be
// changed at runtime, so they live as plain variables instead.
static uint32_t idleLockMs   = 60000;   // auto-lock after inactivity (0 = never)
static uint8_t  typeDelayMs  = 6;       // per-keystroke delay, HID reliability
static uint8_t  genPwLength  = 20;      // default generated-password length
static bool     typeEnter    = false;   // press Enter after typing a password?
...
/* ----------------------------------------------------------------------------
 *  7b. SETTINGS  —  non-secret prefs, saved as a plain little file on the card
 * -------------------------------------------------------------------------- */
...
static void saveSettings() {
  ...
  if (!f) return;
  f.printf("idle=%lu\n", (unsigned long)idleLockMs);
  f.printf("typedelay=%u\n", typeDelayMs);
  f.printf("genlen=%u\n", genPwLength);
  f.printf("enter=%u\n", typeEnter ? 1 : 0);
  f.close();
}
...
static void loadSettings() {
  ...
  if (!f) return;                       // no file yet: keep the defaults
  while (f.available()) {
    ...
    if      (k == "idle")      idleLockMs  = (uint32_t)v;
    else if (k == "typedelay") typeDelayMs = constrain((int)v, 0, 50);
    else if (k == "genlen")    genPwLength = constrain((int)v, 8, 64);
    else if (k == "enter")     typeEnter   = (v != 0);
  }
  f.close();
}
...
enum Screen { SC_SETPIN, SC_LOCK, SC_LIST, SC_ACTIONS, SC_REVEAL, SC_ADD,
              SC_SETTINGS, SC_TOAST };
...
int  settingsIndex = 0;     // which row in the settings menu
bool changingPin   = false; // true = the PIN screen is setting a NEW pin, not unlocking
...
// Flipper-ish status bar: name on the left, a lock glyph + entry count on the
// right. Lives in the top 9px; everything else draws below it.
static void drawStatusBar(const char* title) {
  oled.setFont(u8g2_font_4x6_tf);
  oled.drawStr(2, 6, title);
  // right side: little closed-lock + "n/total"
  oled.drawXBMP(96, 0, ic_lock_small_width, ic_lock_small_height, ic_lock_small_bits);
  if (!vault.empty() && selectedIndex < (int)vault.size()) {
    char c[12];
    snprintf(c, sizeof(c), "%d/%d", selectedIndex + 1, (int)vault.size());
    oled.setFont(u8g2_font_4x6_tf);
    oled.drawStr(108, 6, c);
  }
  oled.drawHLine(0, 8, 128);
}

// each entry gets an icon based on the first letter of its label, so the
// carousel doesn't look like the same key stamped over and over. Purely
// cosmetic — cycles through the glyph set.
static const unsigned char* iconForEntry(int i) {
  switch (i % 4) {
    case 0: return ic_key_bits;
    case 1: return ic_lock_small_bits;
    case 2: return ic_user_bits;
    default: return ic_settings_bits;
  }
}
...
static void drawList(uint32_t now) {
  ...
  if (vault.empty()) {
    oled.drawXBMP(20, 12, ic_plus_width, ic_plus_height, ic_plus_bits);
    oled.setFont(u8g2_font_6x12_tf);
    oled.drawStr(42, 20, "No entries");
    oled.setFont(u8g2_font_4x6_tf);
    oled.drawStr(42, 29, "RECORD to add one");
    return;
  }

  // The carousel has one extra virtual card past the last entry: Settings.
  // Scroll right off the end and you land on it. No spare button needed.
  int total = (int)vault.size() + 1;
  int settingsCard = (int)vault.size();   // index of the Settings card

  for (int i = 0; i < total; i++) {
    float rel = i - carousel;
    if (fabs(rel) > 1.4f) continue;
    int cx = 64 + (int)(rel * 128);
    bool centre = fabs(rel) < 0.5f;

    if (i == settingsCard) {
      oled.drawXBMP(cx - 52, 12, ic_settings_width, ic_settings_height, ic_settings_bits);
      oled.setFont(u8g2_font_7x13B_tf);
      oled.drawStr(cx - 30, 25, "Settings");
    } else {
      oled.drawXBMP(cx - 52, 12, ic_key_width, ic_key_height, iconForEntry(i));
      oled.setFont(u8g2_font_7x13B_tf);
      const char* label = vault[i].label.c_str();
      if (centre) drawMarquee(label, cx - 30, 25, 86, now);
      else        oled.drawStr(cx - 30, 25, label);
    }
  }
}
...
static const unsigned char* ACTION_ICONS[] = {
  ic_key_bits, ic_user_bits, ic_eye_bits, ic_trash_bits, ic_lock_small_bits
};

static void drawActions(uint32_t now) {
  drawStatusBar(vault[selectedIndex].label.c_str());

  // two rows visible below the status bar, each with an icon + label. The
  // selected row is an inverted rounded pill (the iOS/Flipper hybrid look).
  // The window follows actionIndex so the selection is always on screen.
  // Rows are 11px; icons are 16px so we only ever show two rows at once and
  // clip each icon to its row with a per-row clip window. Keeps a tall glyph
  // from bleeding into its neighbour.
  const int rowH = 11;
  int viewTop = 0;
  if (actionIndex >= 2) viewTop = (actionIndex - 1) * rowH;  // keep sel in view

  for (int i = 0; i < N_ACTIONS; i++) {
    int y = 9 + i * rowH - viewTop;
    if (y + rowH < 9 || y > 31) continue;

    if (i == actionIndex) {
      oled.drawRBox(0, y, 122, rowH - 1, 2);
      oled.setDrawColor(0);
    }
    // clip the icon to this row so its full 16px height can't spill over
    oled.setClipWindow(2, y, 20, y + rowH - 1);
    oled.drawXBMP(2, y - 2, ic_key_width, ic_key_height, ACTION_ICONS[i]);
    oled.setMaxClipWindow();

    oled.setFont(u8g2_font_6x10_tf);
    oled.drawStr(22, y + 9, ACTIONS[i]);
    if (i == actionIndex) oled.setDrawColor(1);
  }

  // tiny scrollbar on the right so you know there's more below
  int barH = 24 * N_ACTIONS > 0 ? (24 / N_ACTIONS) : 24;
  int barY = 9 + (actionIndex * (24 - barH)) / (N_ACTIONS - 1);
  oled.drawVLine(126, 9, 22);
  oled.drawBox(125, barY, 3, barH);
}

// ---- settings menu -----------------------------------------------------------
// Rows: 0 Change PIN, 1 Auto-lock, 2 Type speed, 3 PW length, 4 Type Enter, 5 Back
// On an adjustable row, SCROLL L/R changes the value live. SELECT on "Change
// PIN" jumps into the PIN setter; SELECT on "Back" saves + returns to the list.
enum { SET_PIN, SET_IDLE, SET_SPEED, SET_LEN, SET_ENTER, SET_BACK, N_SETTINGS };
static const char* SETTING_NAMES[] = {
  "Change PIN", "Auto-lock", "Type speed", "PW length", "Type Enter", "Back"
};

// human-readable current value for the row on the right
static void settingValue(int row, char* buf, size_t n) {
  switch (row) {
    case SET_IDLE:
      if (idleLockMs == 0) snprintf(buf, n, "Off");
      else                 snprintf(buf, n, "%lus", (unsigned long)(idleLockMs / 1000));
      break;
    case SET_SPEED: snprintf(buf, n, "%ums", typeDelayMs); break;
    case SET_LEN:   snprintf(buf, n, "%u", genPwLength);   break;
    case SET_ENTER: snprintf(buf, n, typeEnter ? "Yes" : "No"); break;
...
static void drawSettings(uint32_t now) {
  ...
  const int rowH = 11;
  int viewTop = 0;
  if (settingsIndex >= 2) viewTop = (settingsIndex - 1) * rowH;
  ...
  for (int i = 0; i < N_SETTINGS; i++) {
    ...
    if (sel) { oled.drawRBox(0, y, 122, rowH - 1, 2); oled.setDrawColor(0); }

    oled.setFont(u8g2_font_6x10_tf);
    oled.drawStr(4, y + 9, SETTING_NAMES[i]);

    // value on the right, with little ‹ › arrows when it's the selected,
    // adjustable row — telegraphs "you can change this with scroll"
    char v[12]; settingValue(i, v, sizeof(v));
    if (v[0]) {
      int vw = oled.getStrWidth(v);
      int vx = 118 - vw;
      if (sel && i != SET_PIN && i != SET_BACK) {
        oled.drawStr(vx - 7, y + 9, "<");
        oled.drawStr(119, y + 9, ">");
      }
      oled.drawStr(vx, y + 9, v);
    }
    if (sel) oled.setDrawColor(1);
  }

  int barH = 24 / N_SETTINGS;
  int barY = 9 + (settingsIndex * (24 - barH)) / (N_SETTINGS - 1);
  oled.drawVLine(126, 9, 22);
  oled.drawBox(125, barY, 3, barH);
}
...
static void addModeCommit(const String& line) {
  ...
  if (pw == "*")                 pw = generatePassword(genPwLength);
...
static void submitPin() {
  // Changing an existing PIN from Settings: capture twice, then re-encrypt the
  // (already-decrypted) vault under the new PIN.
  if (changingPin) {
    if (pinPass == 0) {
    ...
    changingPin = false;
    saveVault();                                  // re-encrypts with new key
    screen = SC_LIST;
    toast("PIN changed", 1000, SC_LIST);
    return;
  }
...
static void handleList() {
  int total = (int)vault.size() + 1;   // +1 for the Settings card
  int settingsCard = (int)vault.size();

  if (took(B_LEFT))  { selectedIndex = (selectedIndex + total - 1) % total; bumpActivity(); }
  if (took(B_RIGHT)) { selectedIndex = (selectedIndex + 1) % total; bumpActivity(); }
  ...
  bool onSettings = (selectedIndex == settingsCard);

  if (!onSettings && !vault.empty() && took(B_PLAY)) {   // quick-type password
    bumpActivity();
    typeString(vault[selectedIndex].password);
    if (typeEnter) Keyboard.write('\n');
    toast("Typed", 700, SC_LIST);
  }
  if (took(B_SELECT)) {
    bumpActivity();
    if (onSettings) { settingsIndex = 0; screen = SC_SETTINGS; }
    else if (!vault.empty()) { actionIndex = 0; pillY = 0; screen = SC_ACTIONS; }
  }
}
...
// Kick off setting a brand-new PIN (from Settings > Change PIN). Re-uses the
// PIN entry screen; `changingPin` tells submitPin to re-encrypt instead of
// unlock. We keep the current vault decrypted in RAM the whole time.
static void beginChangePin() {
  changingPin = true;
  pinPass = 0; pinCursor = 0;
...
static void handleSettings() {
  if (took(B_RECORD)) {                        // back out, save prefs
    saveSettings(); screen = SC_LIST; bumpActivity(); return;
  }

  bool valueRow = (settingsIndex == SET_IDLE || settingsIndex == SET_SPEED ||
                   settingsIndex == SET_LEN  || settingsIndex == SET_ENTER);

  // Only LEFT/RIGHT/SELECT are free here, so behaviour splits by row type:
  //   value row  -> LEFT/RIGHT tweak the value, SELECT moves to the next row
  //   PIN / Back -> LEFT/RIGHT move between rows, SELECT activates the row
  // That keeps everything reachable with the buttons you have.
  if (valueRow && took(B_LEFT)) {
    switch (settingsIndex) {
      case SET_IDLE:  idleLockMs  = (idleLockMs <= 15000) ? 0 : idleLockMs - 15000; break;
      case SET_SPEED: typeDelayMs = (typeDelayMs <= 2) ? 0 : typeDelayMs - 2;       break;
      case SET_LEN:   genPwLength = (genPwLength <= 8) ? 8 : genPwLength - 2;        break;
      case SET_ENTER: typeEnter   = !typeEnter;                                      break;
    }
    bumpActivity();
  }
  if (valueRow && took(B_RIGHT)) {
    switch (settingsIndex) {
      case SET_IDLE:  idleLockMs  = (idleLockMs >= 300000) ? 300000 : idleLockMs + 15000; break;
      case SET_SPEED: typeDelayMs = (typeDelayMs >= 30) ? 30 : typeDelayMs + 2;           break;
      case SET_LEN:   genPwLength = (genPwLength >= 64) ? 64 : genPwLength + 2;           break;
      case SET_ENTER: typeEnter   = !typeEnter;                                           break;
    }
    bumpActivity();
  }
  if (!valueRow && took(B_LEFT))  { settingsIndex = (settingsIndex + N_SETTINGS - 1) % N_SETTINGS; bumpActivity(); }
  if (!valueRow && took(B_RIGHT)) { settingsIndex = (settingsIndex + 1) % N_SETTINGS; bumpActivity(); }

  if (took(B_SELECT)) {
    bumpActivity();
    if (settingsIndex == SET_PIN)       beginChangePin();
    else if (settingsIndex == SET_BACK) { saveSettings(); screen = SC_LIST; }
    else settingsIndex = (settingsIndex + 1) % N_SETTINGS;
  }
}
...
// Animated boot logo, Flipper-style. The padlock eases in from the left, the
// "DSH-AEGIS" wordmark slides in beside it, then a little scanning bar sweeps
// underneath before we hand off to the lock screen. Blocking on purpose — it
// only runs once at power-on.
static void bootSplash() {
  const int lockX = 8, lockY = 5;                 // final resting spot for logo
  const int textTargetX = 44;

  // phase 1: lock drops in from above with an ease-out
  for (int f = 0; f <= 14; f++) {
    float t = f / 14.0f;
    float e = 1 - (1 - t) * (1 - t);              // easeOutQuad
    int y = (int)(-24 + e * (lockY + 24));
    oled.clearBuffer();
    oled.drawXBMP(lockX, y, logo_lock_width, logo_lock_height, logo_lock_bits);
    oled.sendBuffer();
    delay(14);
  }

  // phase 2: wordmark slides in from the right, settling next to the lock
  oled.setFont(u8g2_font_7x13B_tf);
  for (int f = 0; f <= 12; f++) {
    float t = f / 12.0f;
    float e = 1 - (1 - t) * (1 - t) * (1 - t);    // easeOutCubic
    int x = (int)(128 - e * (128 - textTargetX));
    oled.clearBuffer();
    oled.drawXBMP(lockX, lockY, logo_lock_width, logo_lock_height, logo_lock_bits);
    oled.setFont(u8g2_font_7x13B_tf);
    oled.drawStr(x, 16, "DSH");
    oled.drawStr(x, 29, "AEGIS");
    oled.sendBuffer();
    delay(16);
  }

  // phase 3: a scanning bar sweeps under the wordmark, then hold
  for (int f = 0; f <= 20; f++) {
    oled.clearBuffer();
    oled.drawXBMP(lockX, lockY, logo_lock_width, logo_lock_height, logo_lock_bits);
    oled.setFont(u8g2_font_7x13B_tf);
    oled.drawStr(textTargetX, 16, "DSH");
    oled.drawStr(textTargetX, 29, "AEGIS");
    int sw = (f * 84) / 20;
    oled.drawHLine(textTargetX, 31, sw);
    oled.sendBuffer();
    delay(18);
  }
  delay(300);
...
static void lockDevice() {
  ...
  vault.clear();
  memset(masterPin, 0, PIN_LENGTH);
  unlocked = false; addMode = false; changingPin = false;
  pinCursor = 0; pinPass = 0; settingsIndex = 0;
  for (int i = 0; i < PIN_LENGTH; i++) pinDigits[i] = 0;
...
void setup() {
  ...
  bootSplash();   // animated logo — see below
  ...
  screen = SD.exists(VAULT_PATH) ? SC_LOCK : SC_SETPIN;
  loadSettings();                       // pull saved prefs off the card
  for (int i = 0; i < PIN_LENGTH; i++) pinDigits[i] = 0;
  bumpActivity();
}
...
void loop() {
  ...
  // auto-lock on idle (only once we're actually in; 0 = never)
  if (unlocked && screen != SC_ADD && screen != SC_SETPIN && idleLockMs > 0 &&
      now - lastActivity > idleLockMs) {
    lockDevice();
  }
  ...
  switch (screen) {
    ...
    case SC_SETTINGS:  handleSettings(); break;
  ...
  switch (screen) {
    case SC_SETPIN:
      drawPinScreen(changingPin ? (pinPass == 0 ? "New PIN" : "Confirm new")
                                : (pinPass == 0 ? "Set PIN" : "Confirm PIN"), now);
      break;
    ...
    case SC_SETTINGS: drawSettings(now); break;
